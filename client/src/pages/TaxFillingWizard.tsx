...
import { Loader2, FileDown, Calculator, ArrowRight, CheckCircle2, FileSpreadsheet } from "lucide-react"
...
export default function TaxFillingWizard() {
  ...
  const fetchAISuggestions = async () => {
    try {
      ...
      const { data } = await axios.get(`${API_BASE_URL}/api/tax/ai-suggestions`, {
        headers: {
    ...
    <div className="container mx-auto py-8 px-4">
      <Card className="max-w-2xl mx-auto border-teal-100 dark:border-teal-900/50 shadow-md">
        <CardHeader className="bg-gradient-to-r from-teal-50 to-emerald-50 dark:from-teal-900/20 dark:to-emerald-900/20 border-b border-teal-100 dark:border-teal-900/50">
          <CardTitle className="text-2xl text-teal-800 dark:text-teal-300">Tax Filing Wizard</CardTitle>
              ...
              <div key={i} className="flex flex-col items-center">
                ...
                <span
                  className={`mt-2 text-xs ${
                    i <= step ? "text-teal-600 dark:text-teal-400 font-medium" : "text-zinc-500"
                  }`}
                >
                  {s.title}
                </span>
                {i < steps.length - 1 && (
                  <div
                    className={`hidden md:block h-[2px] w-16 ${
                      i < step ? "bg-teal-600 dark:bg-teal-400" : "bg-zinc-200 dark:bg-zinc-700"
                    } absolute left-[calc(${(i + 0.5) * (100 / steps.length)}% - 8px)]`}
                  />
                )}
              </div>
            ))}
          ...
          <div className="mt-6">
            ...
            {step === 1 && (
              <div className="p-6 bg-teal-50 dark:bg-teal-900/20 rounded-md border border-teal-100 dark:border-teal-900/50 text-center">
                ...
                <h3 className="text-lg font-medium text-teal-700 dark:text-teal-300 mb-2">Ready to Calculate</h3>
                <p className="text-zinc-600 dark:text-zinc-400 mb-4">
                  Your CSV has been uploaded successfully. Click next to calculate tax based on your data.
                </p>
              </div>
            )}
                ...
                {fetchingSuggestions ? (
                  <div className="p-4 bg-teal-50 dark:bg-teal-900/20 rounded-md border border-teal-100 dark:border-teal-900/50 animate-pulse">
                    <div className="flex items-center">
                      ...
                      <p className="text-teal-700 dark:text-teal-300">Generating AI suggestions...</p>
                    </div>
                  </div>
                ) : suggestions ? (
                  <Card className="border-teal-100 dark:border-teal-900/50">
                    ...
                    <CardContent>
                      <div className="text-sm whitespace-pre-line text-zinc-700 dark:text-zinc-300">{suggestions}</div>
                    </CardContent>
                  </Card>
                ) : null}
